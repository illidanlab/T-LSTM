...
class T_LSTM_AE(object):
    ...
    def __init__(self, input_dim, output_dim, output_dim2, output_dim3, hidden_dim, hidden_dim2, hidden_dim3):
        ...
        # self.target = tf.placeholder('float', shape=[None, None, self.output_dim3])
    ...
    def T_LSTM_Encoder_Unit(self, prev_hidden_memory, concat_input):
        ...
        prev_cell_decompose = tf.nn.tanh(tf.matmul(prev_cell, self.W_decomp_enc) + self.b_decomp_enc)
        # if T is 0, then the weight is one
        prev_cell = prev_cell - prev_cell_decompose + tf.mul(T, prev_cell_decompose)
        # prev_cell = tf.mul(T, prev_cell)
    ...
    def T_LSTM_Encoder_Unit2(self, prev_hidden_memory, concat_input):
        ...
        # Decompose the previous cell if there is a elapse time
        prev_cell_decompose = tf.nn.tanh(tf.matmul(prev_cell, self.W_decomp_enc2) + self.b_decomp_enc2)
        # if T is 0, then the weight is one
        prev_cell = prev_cell - prev_cell_decompose + tf.mul(T, prev_cell_decompose)
        # prev_cell = tf.mul(T, prev_cell)
    ...
    def T_LSTM_Decoder_Unit(self, prev_hidden_memory, concat_input):
        ...
        # Decompose the previous cell if there is a elapse time
        prev_cell_decompose = tf.nn.tanh(tf.matmul(prev_cell, self.W_decomp_dec) + self.b_decomp_dec)
        # if T is 0, then the weight is one
        prev_cell = prev_cell - prev_cell_decompose + tf.mul(T, prev_cell_decompose)
        # prev_cell = tf.mul(T, prev_cell)
    ...
    def T_LSTM_Decoder_Unit2(self, prev_hidden_memory, concat_input):
        ...
        # Decompose the previous cell if there is a elapse time
        prev_cell_decompose = tf.nn.tanh(tf.matmul(prev_cell, self.W_decomp_dec2) + self.b_decomp_dec2)
        # if T is 0, then the weight is one
        prev_cell = prev_cell - prev_cell_decompose + tf.mul(T, prev_cell_decompose)
        # prev_cell = tf.mul(T, prev_cell)
    ...
    def get_encoder2_states(self):
        ...
        # make scan_time [seq_length x batch_size x 1]
        scan_time = tf.reshape(scan_time, [tf.shape(scan_time)[0], tf.shape(scan_time)[1], 1])
        concat_input = tf.concat(2, [scan_time, encoder1_outputs])  # [seq_length x batch_size x input_dim+1]
    ...
    def get_representation(self):
        ...
        representation = tf.reverse(all_encoder2_states, [True, False, False])[0, :, :]
        decoder_ini_cell = tf.reverse(all_encoder2_cells, [True, False, False])[0, :, :]
    ...
    def get_decoder2_states(self):
        ...
        # make scan_time [seq_length x batch_size x 1]
        scan_time = tf.reshape(scan_time, [tf.shape(scan_time)[0], tf.shape(scan_time)[1], 1])
        concat_input = tf.concat(2, [scan_time, decoder1_outputs])  # [seq_length x batch_size x input_dim+1]
    ...
    def get_decoder_outputs(self): # Returns the output of only the last time step
        ...
        reversed_outputs = tf.reverse(all_outputs, [True, False, False])
